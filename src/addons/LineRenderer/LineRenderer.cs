...
public partial class LineRenderer : MeshInstance3D {
    [Export] public Godot.Collections.Array<Vector3> points = [new Vector3(0, 0, 0), new Vector3(0, 5, 0)];
    ...
    private void Corner(Vector3 center, Vector3 start, Vector3 end, int smoothing) {
        ...
        for (int i = 0; i <= smoothing; i++) {
            ...
            Vector3 offset = axis.Rotated(axis.Cross(end - center).Normalized(), angle) * (start - center).Length();
