...
public partial class MatrixControl : Control {
    ...
    private new void UpdateMinimumSize() {
        ...
        int totalRows = _grids.Count > 0 ? _grids.Sum(g => g?.Rows ?? 0) : 0;
        CustomMinimumSize = new Vector2(maxColumns * CellSize, totalRows * CellSize);
    }
    ...
    private void DrawGrid(GridConfiguration grid, float offsetX) {
        var totalWidth = grid.Columns * CellSize + Padding;
        var totalHeight = grid.Rows * CellSize + Padding;
    ...
    private bool HandleMouseInput(InputEventMouseButton mouseEvent, GridConfiguration grid, float offsetX) {
        var totalWidth = grid.Columns * CellSize + Padding * 2;
        var totalHeight = grid.Rows * CellSize + Padding * 2;
        var offsetY = (Size.Y - totalHeight) / 2;
